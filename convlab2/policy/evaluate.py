...
def evaluate(dataset_name, model_name, load_path, calculate_reward=True):
    ...
    if dataset_name == 'MultiWOZ':
        ...
        elif model_name == "RulePolicy":
            policy_sys = RulePolicy(character='sys')
        ...
        for seed in range(1000):
            ...
            for i in range(40):
                ...
                if session_over is True:
                    ...
                    print(f'task success: {task_succ}')
                    print(f'book rate: {sess.evaluator.book_rate()}')
                    print(f'inform precision/recall/f1: {sess.evaluator.inform_F1()}')
        ...
        for key in task_success: 
            print(f'{key} {len(task_success[key])} {np.average(task_success[key]) if len(task_success[key]) > 0 else 0}')
        ...
        if calculate_reward:
            ...
            for seed in range(1000):
                ...
                mask = []
                for t in range(40):
                    #s_vec = torch.Tensor(policy_sys.vector.state_vectorize(s)[0])
                    ...
                    if done: # one due to counting from 0, the one for the last turn
                        break
                print(f'{seed} reward: {np.mean(reward)}')
                reward_tot.append(np.mean(reward))
            print(f'total avg reward: {np.mean(reward_tot)}')
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_path", type=str, default= path, help="path of model")
