...
class MultiDiscretePolicy(nn.Module):
    def __init__(self, s_dim, h_dim, a_dim):
        ...
        self.bs = nn.Sequential(nn.Linear(h_dim, 50),
                                 nn.ReLU(),
                                 nn.Linear(50, 24))
        self.ua = nn.Sequential(nn.Linear(h_dim, 150),
                                 nn.ReLU(),
                                 nn.Linear(150, 78))
    ...
    def forward(self, s):
        # [b, s_dim] => [b, a_dim]
        a_weights = self.net(s)
        bel_s = self.bs(a_weights)
        u_act = self.ua(a_weights)
        a_weight = self.net1(a_weights)

        return a_weight,bel_s,u_act
...
class MultiDiscretePolicy2(nn.Module):
    def __init__(self, s_dim, h_dim, a_dim):
        super(MultiDiscretePolicy2, self).__init__()
...
Transition = namedtuple('Transition', ('state', 'action', 'reward', 'next_state', 'mask', 'act_cor1','act_cor2'))
