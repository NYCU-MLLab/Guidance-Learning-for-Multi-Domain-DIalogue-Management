...
from convlab2.policy.rlmodule import MultiDiscretePolicy1,MultiDiscretePolicy2, Value
...
class PPO(Policy):
    ...
    def __init__(self, is_train=False, dataset='Multiwoz'):
        ...
        self.multi_entropy_loss = nn.BCEWithLogitsLoss(pos_weight=5* torch.ones(209).to(device=DEVICE))
        if is_train:
        ...
        if dataset == 'Multiwoz':
            ...
            self.policy = MultiDiscretePolicy1(self.vector.state_dim, cfg['h_dim'], self.vector.da_dim).to(device=DEVICE)
    ...
    def predict(self, state):
        ...
        """
        s_vec = torch.Tensor(self.vector.state_vectorize(state)[0])
    ...
    def update(self, epoch, batchsz, s, a, r, mask,act_w,act_t):
        ...
        for i in range(self.update_round):
            ...
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, log_pi_old_sa_shuf = v_target[perm], A_sa[perm], s[perm], a[perm], \
                                                                           log_pi_old_sa[perm]
            act_w_shuf,act_t_shuf = act_w[perm],act_t[perm]
            ...
            v_target_shuf, A_sa_shuf, s_shuf, a_shuf, log_pi_old_sa_shuf = torch.chunk(v_target_shuf, optim_chunk_num), \
                                                                           ...
                                                                           torch.chunk(log_pi_old_sa_shuf,
                                                                                       optim_chunk_num)
            act_w_shuf,act_t_shuf = torch.chunk(act_w_shuf, optim_chunk_num), \
                                    torch.chunk(act_t_shuf, optim_chunk_num)
            # 3. iterate all mini-batch to optimize
            policy_loss, value_loss = 0., 0.
            for v_target_b, A_sa_b, s_b, a_b, log_pi_old_sa_b, act_w_b, act_t_b in zip(v_target_shuf, A_sa_shuf, s_shuf, a_shuf,
                                                                     log_pi_old_sa_shuf,act_w_shuf,act_t_shuf):
                ...
                index = torch.where((act_t_b==torch.zeros(209,).cuda()).all(axis=1)==False)
                #if i == 3:
                if len(index[0])!=0:
                    act_w_b = act_w_b[index]
                    act_t_b = act_t_b[index]
                    loss_a = self.multi_entropy_loss(act_w_b , act_t_b)
                    policy_loss+=loss_a
                    surrogate+=loss_a #add supervised loss to surrogate
                # backprop
                surrogate.backward()
