...
#from ConvLab_domain_prev_classification_copy.convlab2 import policy
from convlab2.nlu import NLU
from convlab2.dst import DST
from convlab2.policy import Policy
from convlab2.nlg import NLG
from copy import deepcopy
from convlab2.policy.rlmodule import MultiDiscretePolicy3,MultiDiscretePolicy4
import torch
import os
...
class PipelineAgent(Agent):
    ...
    def __init__(self, nlu: NLU, dst: DST, policy: Policy, nlg: NLG, name: str):
        ...
        self.domain = ''
        # Hirarki HITL
        self.path =  'path to hotel weight'
        self.path1 = 'path to restaurant weight'
        self.path2 = 'path to attraction weight'
        self.path3 = 'path to train weight'
        self.path4 = 'path to other weight'
    ...
    def response(self, observation):#z edited
        ...
        if self.dst is not None:
            if self.name is 'sys':
                self.dst.state['user_action'] = self.input_action #z edited, asli:self.input_action
            else:
                self.dst.state['system_action'] = self.input_action
            state = self.dst.update(self.input_action)#z edited, asli:self.input_action 
        else:
            state = self.input_action
        state = deepcopy(state) # get rid of reference problem
        # if self.name=='user' and len(self.history)<=1:
        #     self.domain = list(self.policy.policy.domain_goals)[0]

        # get action
        if self.name=='sys' and len(self.history)==1:

            policy_mdl = "path to high-level weight"
            dom_pol = MultiDiscretePolicy3(78, 100,5).to(device=DEVICE)
            dom_pol.load_state_dict(torch.load(policy_mdl, map_location=DEVICE))
            s_vec = torch.Tensor(vector.state_vectorize(state)[0])
            output = dom_pol(torch.unsqueeze(s_vec[:78],0).to(device=DEVICE)).cpu()
            domain = output.argmax(dim=1, keepdim=True)

        
            if domain == 0: 
                self.policy.load(self.path)
            elif domain == 1: 
                self.policy.load(self.path1)
            elif domain == 2: 
                self.policy.load(self.path2)
            elif domain == 3: 
                self.policy.load(self.path3)
            else: 
                self.policy.load(self.path4)
