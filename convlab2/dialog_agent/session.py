...
class BiSession(Session):
    ...
    def next_agent(self):
        ...
        if self.__turn_indicator % 2 == 0:
            ...
            # self.sys_agent.domain = self.user_agent.domain
            next_agent = self.sys_agent
        self.__turn_indicator += 1
        return next_agent

    # def next_response(self, observation,z):#z edited
    #     next_agent = self.next_agent()
    #     response = next_agent.response(observation,z) #z edited
    #     return response

    def next_response(self, observation):#z edited
        next_agent = self.next_agent()
        response = next_agent.response(observation) #z edited
        return response
    ...
    def next_turn(self, last_observation):
        ...
        """
        # user_response = self.next_response(last_observation,None)#none editan
        user_response = self.next_response(last_observation)#none editan
        self.usr_out = self.user_agent.get_out_da()#self.usr_out edited , aslinya ndak ada
        if self.evaluator:
        ...
        # sys_response = self.next_response(user_response,self.usr_out)#self.usr_out edited
        sys_response = self.next_response(user_response)#self.usr_out edited
